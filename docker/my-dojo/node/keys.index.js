...
const bitcoinNetwork =
	process.env.COMMON_BTC_NETWORK === "testnet" ? "testnet" : "bitcoin";
...
if (process.env.SOROBAN_INSTALL === "on") {
	...
	if (
		process.env.SOROBAN_ANNOUNCE === "on" &&
		process.env.NODE_PANDOTX_PROCESS === "on"
	) {
...
export default {
	...
	[bitcoinNetwork]: {
			...
			strategies: {
				...
				auth47: {
					...
					paymentCodes: process.env.NODE_PAYMENT_CODE
						? process.env.NODE_PAYMENT_CODE.split(",").map((str) => str.trim())
						: [],
				},
			},
		 ...
		 */
		addrFilterThreshold: Number.parseInt(
			process.env.NODE_ADDR_FILTER_THRESHOLD,
			10,
		),
		/*
		...
		addrDerivationPool: {
			...
			minNbChildren: Number.parseInt(
				process.env.NODE_ADDR_DERIVATION_MIN_CHILD,
				10,
			),
			// Max number of child processes allowed
			maxNbChildren: Number.parseInt(
				process.env.NODE_ADDR_DERIVATION_MAX_CHILD,
				10,
			),
			// Max duration
			...
			thresholdParallelDerivation: Number.parseInt(
				process.env.NODE_ADDR_DERIVATION_THRESHOLD,
				10,
			),
		},
		/*
		...
		txsScheduler: {
			...
			maxDeltaHeight: Number.parseInt(
				process.env.NODE_TXS_SCHED_MAX_DELTA_HEIGHT,
				10,
			),
		},
		/*
		...
		tracker: {
			...
			mempoolProcessPeriod: Number.parseInt(
				process.env.NODE_TRACKER_MEMPOOL_PERIOD,
				10,
			),
			// Processing of unconfirmed transactions (periodicity in ms)
			unconfirmedTxsProcessPeriod: Number.parseInt(
				process.env.NODE_TRACKER_UNCONF_TXS_PERIOD,
				10,
			),
		},
	},
