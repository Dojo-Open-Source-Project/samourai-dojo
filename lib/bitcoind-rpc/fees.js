...
/**
 * @typedef {import('@dojo-tools/next-block-estimator/dist/types').Result} EstimatorData
 */
...
class Fees {
	...
	async getFees() {
		try {
			if (latestBlock.height > this.block) await this.refresh();
	 ...
	 */
	getEstimatorFees() {
		if (this.estimatorData.ready === false)
			throw errors.estimator.NOT_AVAILABLE;
	...
	async refresh() {
		...
		try {
			const requests = Object.keys(this.fees)
				.map(Number)
				.map((target) => {
					return {
			...
			for (const fee of responses) {
				const feerate =
					fee.result.errors && fee.result.errors.length > 0
						? 1
						: Math.round(fee.result.feerate * 1e5);
