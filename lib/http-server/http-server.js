...
class HttpServer {
	...
	constructor(port, host) {
		...
		this.app = new App({
			noMatchHandler: (req, res) => {
				Logger.error(
					null,
					`HttpServer : 404 - Not found: ${req.method} - ${req.hostname}${req.path}`,
				);
	 ...
	 */
	static setJSONResponse(_req, res, next) {
	 ...
	 * @param {NextFunction} next - next middleware
	 */
	static setCrossOrigin(_req, res, next) {
	 ...
	 * @param {NextFunction} next - next middleware
	 */
	static setConnection(_req, res, next) {
