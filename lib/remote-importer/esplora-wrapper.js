...
class EsploraWrapper extends Wrapper {
	...
	async _getTxsForAddress(address, lastSeenTxid) {
		...
		if (lastSeenTxid) uri = `${uri}/chain/${lastSeenTxid}`;
	...
	async getAddress(address, filterAddr) {
		...
		while (true) {
			...
			if (txids.length < EsploraWrapper.NB_TXS_PER_PAGE) {
				...
				Logger.info(
					`Importer : Import of ${returnValue.address} rejected (too many transactions - ${returnValue.ntx})`,
				);
				returnValue.txids = [];
				returnValue.ntx = 0;
				return returnValue;
	...
	async getChainTipHeight() {
		...
		if (result != null) chainTipHeight = Number.parseInt(result, 10);
