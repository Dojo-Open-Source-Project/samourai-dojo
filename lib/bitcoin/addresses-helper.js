...
class AddressesHelper {
	 ...
	 * @param {Uint8Array} pubKeyBuffer - Buffer storing a public key
	...
	getScriptHashFromBech32(str) {
		try {
			return toHex(bitcoin.address.fromBech32(str).data);
		} catch (error) {
	 ...
	 * @param {Uint8Array} scriptpubkey - scriptpubkey
	...
	outputScript2Address(scriptpubkey) {
		try {
			if (this.isOPReturnScript(scriptpubkey)) return null;
