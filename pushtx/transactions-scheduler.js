...
class TransactionsScheduler {
    ...
    async schedule(script) {
        ...
        for (let entry of script) {
            ...
            if (entry.hop !== lastHopProcessed && entry.nlocktime < lastLockTimeProcessed) throw errors.pushtx.SCHEDULED_BAD_ORDER
        ...
        for (let entry of script) {
            ...
            Logger.info(`PushTx : Registered scheduled tx ${objectTx.txid} (trigger=${objectTx.trigger})`)
            parentTxid = tx.getId()
