...
class TransactionsBundle {
	...
	async prefilterByInputs() {
		...
		const results = await util.parallelCall(lists, (txs) =>
			this._prefilterByInputs(txs),
		);
		return results.flat();
	}
	...
	async _prefilterByOutputs(txs) {
		...
		return [
			...alreadySeenTXsOfInterest,
			...filteredIndexTxs.map((x) => txs[x]),
		];
	}
	...
	async _prefilterByInputs(txs) {
		...
		for (const index in txs) {
			...
			for (const index_ in tx.tx.ins) {
				...
				if (!indexedInputs[key]) indexedInputs[key] = [];
		...
		const results = await util.parallelCall(lists, (list) =>
			db.getOutputSpends(list),
		);
